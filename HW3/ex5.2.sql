...
SELECT `types`.`name` AS `type`
     ...
     , `trains`.`departure`
     , `trains`.`arrival`
     , round(`types`.`fare_rate` / 1000 * `trains`.`distance`, -2)
    AS `fare`
  FROM `trains`
 INNER JOIN `types` ON `trains`.`type` = `types`.`id`
 INNER JOIN `stations` AS `source_stations` ON `trains`.`source` = `source_stations`.`id`
...
SELECT `trains`.`id`
  ...
  FROM `trains`
 INNER JOIN `types`
    ON `trains`.`type`=`types`.`id`
 INNER JOIN `stations` AS `source_stations` 
    ON `trains`.`source`=`source_stations`.`id`
 INNER JOIN `stations` AS `destination_stations` 
    ON `trains`.`destination`=`destination_stations`.`id`
 INNER JOIN `tickets` 
    ON `trains`.`id`=`tickets`.`train`
 GROUP BY `trains`.`id`
